...
export default function OnlineDot({ className = '' }: Props) {
  return (
    <span className={`relative inline-flex w-2 h-2 ${className}`}>
      <span className="absolute inset-0 rounded-full bg-gold-500 animate-ping opacity-60 motion-reduce:animate-none" />
      <span className="relative inline-flex w-2 h-2 rounded-full bg-gold-500" />
    </span>
  );
}
